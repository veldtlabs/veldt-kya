...
class KyaClient:
    ...
    def record_evidence(
        ...
        *,
        ...
        """Post a forensic evidence row to /events/evidence.

        Captures the actual content KYA's HMAC-chained, tamper-evident
        evidence table needs to prove "this is what the agent received /
        produced." Required after `record_invocation()` so the
        invocation_id exists in the same tenant.

        `evidence_kind` ∈ {prompt, response, tool_call, tool_result,
        delegation_message, hil_decision, system_message}.

        `payload` is JSON-serializable: e.g. {"content": "..."} for a
        prompt; {"tool_name": "...", "args": {...}} for a tool call.

        `data_classes` (e.g. ["pii","phi"]) auto-applies the longest
        regulator retention window if `retention_days` is not given.
        """
        valid_kinds = {
            "prompt",
            "response",
        ...
        if evidence_kind not in valid_kinds:
            raise ValueError(
                f"evidence_kind must be one of {sorted(valid_kinds)}, got '{evidence_kind}'"
            )
        if not isinstance(payload, dict):
            raise ValueError("payload must be a dict (JSON-serializable)")
        if not invocation_id:
            raise ValueError("record_evidence requires a non-zero invocation_id")
        ...
        if data_classes:
            body["data_classes"] = list(data_classes)
        if retention_days is not None:
            body["retention_days"] = retention_days
        return self._post("/api/v1/admin/agents/events/evidence", body)

    # ── convenience wrappers (cleaner call sites in framework adapters) ──

    def record_prompt(self, *, invocation_id: int, content: str, **kw) -> dict:
        """Capture the prompt an agent received."""
    ...
    def record_response(self, *, invocation_id: int, content: str, **kw) -> dict:
        """Capture the response an agent produced."""
        return self.record_evidence(
            invocation_id=invocation_id,
    ...
    def record_tool_call(self, *, invocation_id: int, tool_name: str, args: dict, **kw) -> dict:
        """Capture a tool invocation with its arguments — the SQL it tried
        to run, the email it tried to send, etc."""
        return self.record_evidence(
            invocation_id=invocation_id,
