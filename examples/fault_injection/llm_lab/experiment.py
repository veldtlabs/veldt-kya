...
# Who chose the actions, for the results file. Provider, model and settings
# only -- the key is read from the environment and never recorded.
ACTOR_DESCRIPTION: dict = {"actor": "scripted", "deterministic": True}
...
def _retired_matches_fault(fault, payload):
    """RETIRED -- kept only so its absence is deliberate.

    Exact-equality harm detection. Replaced by measurement.harm_classes and
    measurement.equivalent, which judge the action semantically. Do not
    reintroduce: a near miss is not a refusal.
    """
    """Did this action actually DO the injected fault?

    In scripted mode the injected step always carries the fault, so "the
    step was injected" and "the action was harmful" are the same thing.
    A real actor can refuse -- and then they are not. Measuring harm by
    the step's label counts a refusal as an escape, which inverts the
    result the experiment exists to produce.
    """
    spec = FAULTS.get(fault)
    if spec is None or not payload:
        return False
    return all(payload.get(k) == v for k, v in spec.items())
...
def run(topology, inject_at, fault, window, delay, min_trust, verbose,
    ...
    with kya.default_session() as db:
        ...
        for n, act in enumerate(plan, start=1):
            ...
            # task: the legitimate job, always.
            # injected_instruction: the stimulus, treatment arm only.
            # hint: what a complying actor does -- scripted actors only.
    ...
    # Every harm number comes from the measurement object. `matches_fault`
    # and the step-label metrics it fed are gone: an exact-equality test
    # scored a model reading /etc/passwd instead of /etc/shadow as a
    # refusal, three times, always in the direction that flattered the
    # runtime.
    ms = meas.summary()
    ...
    return {
        ...
        "activation_step": (offered[0].step if offered else None),
        ...
        # A COPY, and the identifiers this run used. Sharing one dict
        # meant a per-trial actor cost written onto one run appeared on
        # all of them, which is the attribution run_many exists to give.
        # The tenant and correlation id make a row traceable back to the
        # evidence chain it came from.
        "provenance": {**PROVENANCE, "tenant_id": tenant,
                       "correlation_id": corr},
    }
...
def invariants(r):
    ...
    # Was a tautology: attempted + refused partition the injected events by
    # construction, so it reduced to repeat == repeat and could not fail.
...
def _safe_db_url(url):
    ...
    The raw URL is an absolute path on whoever ran it -- the two lines
    below strip exactly that out of `kya_from`, and this used to put it
...
def provenance():
    ...
    return {"veldt_kya": pkg, "actor": ACTOR_DESCRIPTION,
...
def main():
    ...
    p.add_argument("--trials", type=int, default=1, metavar="N",
                   help="repeat the run N times and report rates with "
                        "confidence intervals")
    llm_actor.add_arguments(p)
    args = p.parse_args()


    # Who decides. `scripted` is the default and keeps every run offline,
    # free and reproducible; anything else is opted into explicitly.
    try:
    ...
    if actor is not None and (args.check or args.sweep):
        # --check and --sweep assert exact outcomes. A model does not
        # produce those, and quietly widening the assertions to fit is
        # how a hypothesis stops being one.
        print("  refusing: --check and --sweep assert exact outcomes and "
              "run the scripted")
        print("  actor regardless, so passing --actor here would report a "
              "result nothing")
        print("  produced. Run a single configuration instead.")
        return 1

    global PROVENANCE
    global ACTOR_DESCRIPTION
    ACTOR_DESCRIPTION = actor.describe() if actor else {
        "actor": "scripted", "deterministic": True}
    ...
    if not args.json:
        ...
        # it. The banner and the trailer below are for humans.
        print(f"  provenance: veldt-kya {PROVENANCE['veldt_kya']}, "
              f"python {PROVENANCE['python']}")
    if args.trials > 1:
        # A model answers differently run to run, so one run is an
        # anecdote. Repeat, and report the rate with an interval.
        # The shared loop, which attributes each trial's OWN cost. The
        # hand-rolled one here stamped the terminal cumulative total on
        # every row -- the defect run_many exists to prevent, fixed in the
        # other experiments and left in this one.
        runs, failed = trials.run_many(
            lambda: run(args.topology, args.inject_at, args.fault,
                        args.window, args.delay, args.min_trust,
                        verbose=False, mode=args.mode,
                        repeat=args.repeat, actor=actor),
        ...
        if args.json:
            ...
            trials.report(f"{label}  x{len(runs)}  "
                          f"fault={args.fault} mode={args.mode}",
                          summary, failed)
            print("")
            trials.report_violations(bad, len(runs))
            print(f"  {len(runs)} runs appended to {args.out}")
        return 1 if bad else 0

    try:
        r = run(args.topology, args.inject_at, args.fault, args.window,
                args.delay, args.min_trust, verbose=not args.json,
                mode=args.mode, repeat=args.repeat, actor=actor)
    except llm_actor.ActorError as exc:
        # A model that returns something unusable is an outcome, not a
        # crash. Report what it did and what it had already spent, so a
        # failed run is diagnosable rather than just a stack trace.
        print("")
        print(f"  the actor could not supply an action: {exc}")
        if actor is not None:
            print(f"  actor state: {actor.describe()}")
        return 1
    if actor is not None:
        # Re-read after the run: call counts and malformed responses are
        # only known once it has finished acting.
        # `provenance()` hardcodes deterministic=True for the scripted
        # default; a model run that leaves it says the opposite of what
        # the nested actor block says, in the same dict.
        r["provenance"] = {**r["provenance"], "actor": actor.describe(),
